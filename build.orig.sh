...
declare PDF_ENGINE=pdflatex
# Comment out the line below to use pdflatex engine
# PDF_ENGINE=xelatex
...
$(printf '                      %s\n' ${ARG_FONT_SIZES[@]})
  ...
  -short-title      use argument as short title
...
declare -r ARG_PAPER_A4="a4"
declare -r ARG_PAPER_USLETTER="usletter"
...
declare -r PAPER_A4="a4paper"
declare -r PAPER_US_LETTER="letterpaper"

declare -r FONT_SIZE_9PT="9pt"
...
if [[ $# -gt 0 ]] && [[ "${1}" = "-fontsize" ]]; then
    if [[ "${2}" == "${ARG_FONT_SIZE_9PT}" ]]; then
        output_font_size="--metadata=fontsize:${FONT_SIZE_9PT}"
    elif [[ "${2}" == "${ARG_FONT_SIZE_10PT}" ]]; then
...
if [[ $# -gt 0 ]] && [[ "${1}" = "-short-title" ]]; then
    output_short_title="--variable=shorttitle:${2}"
    shift 2
fi

if [[ $# -gt 0 ]] && [[ "${1}" = "-type" ]]; then
    output_doc_type="--variable=type:${2}"
    shift 2
fi
...
if [ ${output_image_generate} -eq 1 ]; then
    ...
    for file in "${tex_files[@]}"; do
        ...
        # Move .png file from ${INPUT_DIR}/tex-images directory to
        # current directory; always force move operation
        # mv -f "${basefilename}.png" .

        [ -e "${basefilename}.aux" ] && mv -f "${basefilename}.aux" ${INPUT_DIR}/tex-images/
        [ -e "${basefilename}.log" ] && mv -f "${basefilename}.log" ${INPUT_DIR}/tex-images/
        [ -e "${basefilename}.pdf" ] && mv -f "${basefilename}.pdf" ${INPUT_DIR}/tex-images/
        [ -e "${basefilename}.png" ] && mv -f "${basefilename}.png" ${INPUT_DIR}/tex-images/
        popd
    done
...
if [ ${output_latex} -eq 1 ]; then
    ...
    ${PROGRAM}                                          \
        ...
        ${include_front_matter}                         \
        ${include_back_matter}                          \
        --resource-path=${INPUT_DIR}:${INPUT_DIR}/tex-images:${TEMPLATE_DIR}    \
...
[ ${flag_debug_mode} -eq 0 ] && echo_debug "Deleting mainmatter intermediate files:"
[ ${flag_debug_mode} -eq 1 ] && echo_debug "Keeping mainmatter intermediate files:"
...
[ ${flag_debug_mode} -eq 0 ] && echo_debug "Deleting image files:"
[ ${flag_debug_mode} -eq 1 ] && echo_debug "Keeping image files:"
pushd "${INPUT_DIR}"
if [ ${flag_debug_mode} -eq 0 ]; then
    if [ -d "images" ]; then
        rm -rf "images"
...
[ ${flag_debug_mode} -eq 0 ] && echo_debug "Deleting TeX image files:"
[ ${flag_debug_mode} -eq 1 ] && echo_debug "Keeping TeX image files:"
if [ ${flag_debug_mode} -eq 0 ]; then
    if [ -d "tex-images" ]; then
        rm -rf "tex-images"
    fi
...
[ ${flag_debug_mode} -eq 0 ] && echo_debug "Deleting frontmatter intermediate files:"
[ ${flag_debug_mode} -eq 1 ] && echo_debug "Keeping frontmatter intermediate files:"
if [ ${output_frontmatter_generate} == 1 ]; then
    for file in "${pp_fm_files[@]}"; do
        if [ ${flag_debug_mode} -eq 0 ]; then
            rm -f "${file}"
...
[ ${flag_debug_mode} -eq 0 ] && echo_debug "Deleting backmatter intermediate files:"
[ ${flag_debug_mode} -eq 1 ] && echo_debug "Keeping backmatter intermediate files:"
if [ ${output_backmatter_generate} == 1 ]; then
